...
| 16 | Conformité workation par **classement** (conforme/à confirmer/non conforme), jamais par exclusion | Anti-pattern #1 : on n'élimine pas un hébergement ; la recommandation privilégie le conforme mais tout reste visible et signalé | Filtre excluant sur bureau/clim/5G (perdrait des options et masquerait des compromis) |
| 17 | Génération IA en **forced tool use** (`tool_choice`) plutôt qu'extraction JSON par regex | Sortie JSON garantie et validable par Zod, compatible SDK 0.104 ; robuste vs parsing de texte libre | `output_config.format` (structured outputs — non typé dans la version SDK installée) ; extraction regex (fragile) |
| 18 | Bureau partagé proche comme **alternative** au poste en chambre (assouplit le late checkout) | Élargit les hébergements viables pour les nuits travaillées sans exclure ; un coworking à ≤ 10 min en skateboard couvre le poste de travail | Exiger le poste de travail **dans** l'hébergement (réduit l'offre, force le late checkout) |
| 19 | Cache TTL (`CachingVehicleProvider`) devant la Fleet API Tesla | La Fleet API facture chaque lecture (statut < 500) ; sondage minute ≈ 10 $/mois/véhicule. Le cache plafonne à une lecture facturable par véhicule/fenêtre (défaut 5 min), sans réveil ni relance facturable | Lire à chaque recalcul d'itinéraire (coût linéaire) ; streaming fleet telemetry (surdimensionné pour une lecture ponctuelle) |
